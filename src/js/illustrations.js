export class Illustrator {
    ...
    generatKernelGraph(kernels) {
        ...
        var plotData = []
        const allTraces = document.getElementById("myDiv").data;
        const trace = allTraces.filter(trace => trace.visible === true);

        for (var i = 0; i < plotting_data.length; i++) {
            let isTraceVisible = true;

            if (trace.length != 0)
                isTraceVisible = trace.some(trace => trace.name === this.plotNames[i]);
    ...
    generatFrequencyGraph(pixels, row) {
        ...
        const allTraces = document.getElementById("myPlot2").data;
        const trace = allTraces.filter(trace => trace.visible === true);
